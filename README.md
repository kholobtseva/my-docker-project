...
В основе проекта лежит мой отлаженный скрипт, который успешно работал в продакшене более 1.5 лет. Я расширила его до полноценной системы, чтобы углубленно изучить микросервисную архитектуру и современные DevOps-практики. Архитектура сознательно сохраняет "неоптимальные" элементы для учебных целей:
...
- Интеграция разнородных систем (API → PostgreSQL → Elasticsearch → Kafka)
