...
### TC-E2E-006: Pipeline Performance Under Sequential Load
**Priority:** Medium  
**Type:** Performance  
**Description:** Проверка устойчивости пайплайна при последовательных запусках с накоплением данных  
**Preconditions:**
- Все сервисы запущены
- Singapore Exchange API доступен

**Test Steps:**
1. Запустить скрипт 5 раз подряд с интервалом 30 секунд:
   - `docker-compose up python-script`
   - `Start-Sleep -Seconds 30`
   - Повторить 4 раза
2. Мониторить потребление ресурсов: `docker stats`
3. Отслеживать логи на предмет ошибок или деградации производительности
4. Проверить итоговые результаты:
   - В PostgreSQL: `SELECT COUNT(*) FROM agriculture_moex;` (должны быть только уникальные записи)
   - В CSV: проверить общее количество строк
   - В Kafka: проверить общее количество обработанных сообщений

**Expected Results:**
- Система стабильно работает при множественных запусках
- ON CONFLICT механизм корректно обрабатывает дублирующиеся данные
- Потребление ресурсов не растет линейно с количеством запусков
- Отсутствуют утечки памяти или накопление ошибок
- Все компоненты сохраняют работоспособность
