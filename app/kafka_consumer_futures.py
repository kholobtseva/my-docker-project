# app/kafka_consumer_futures.py - Consumer для MOEX фьючерсов
import json
import logging
import csv
import os
...
logger = setup_logging('kafka_consumer_futures')
graylog_logger = setup_graylog_logger('kafka_consumer_futures')

# Data validation schema для фьючерсов
DATA_SCHEMA = {
    "type": "object",
    "properties": {
...
class DeadLetterQueueProducer:
    ...
    def send_to_dlq(self, original_message, error_reason, raw_value=None):
        ...
        dlq_message = {
            ...
            "dlq_topic": "futures-data-dlq"
        }
        
        try:
            future = self.producer.send('futures-data-dlq', dlq_message)
...
def save_to_csv(data, filename='kafka_messages_futures.csv'):
    """Save data to CSV file с путем на Windows"""
    try:
...
def run_consumer():
    """Main consumer function for futures data"""
    logger.info("Starting Kafka consumer for futures data", extra={
        'extra_data': {
    ...
    for attempt in range(max_retries):
        try:
            consumer = create_kafka_consumer(
                topic='moex-futures-data',  # Топик для фьючерсов
                group_id='futures_csv_writer_group'
            )
            
            if consumer:
                logger.info("Kafka consumer for futures started", extra={
                    'extra_data': {
                        'event_type': 'consumer_initialized',
                        'status': 'success',
                        'attempt': attempt + 1,
                        'topic': 'moex-futures-data',
                        'group_id': 'futures_csv_writer_group'
                    }
                })
                break
